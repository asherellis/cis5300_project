...
parser = argparse.ArgumentParser(
    description="Evaluate Extension 5 (RAG with rerank with chunking) predictions."
)
parser.add_argument(
    "split",
    nargs="?",
...
extension5_output = f"output/{split}_extension5_output.json"

if not os.path.exists(extension5_output):
    print(f"Error: Output file not found: {extension5_output}", file=sys.stderr)
    sys.exit(1)

print(f"\nEvaluating Extension 5 (RAG with rerank with chunking) on {split} split...")
print(f"Questions file: {questions_file}")
print(f"Predictions file: {extension5_output}\n")

questions = load_jsonl(questions_file)
print(f"Loaded {len(questions)} questions\n")

predictions = load_predictions(extension5_output)
...
all_evaluation = {'rag_extension5': results}

output_file = f"output/{split}_extension5_evaluation.json"
