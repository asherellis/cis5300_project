...
parser = argparse.ArgumentParser(
    description="Evaluate Extension 4 (Hybrid Retrieval + Reranking + LLM Citations) predictions."
)
parser.add_argument(
    "split",
    nargs="?",
...
extension4_output = f"output/{split}_extension4_output.json"

if not os.path.exists(extension4_output):
    print(f"Error: Output file not found: {extension4_output}", file=sys.stderr)
    sys.exit(1)

print(f"\nEvaluating Extension 4 (Hybrid Retrieval + Reranking + LLM Citations) on {split} split...")
print(f"Questions file: {questions_file}")
print(f"Predictions file: {extension4_output}\n")

questions = load_jsonl(questions_file)
print(f"Loaded {len(questions)} questions\n")

predictions = load_predictions(extension4_output)
...
all_evaluation = {'rag_extension4': results}

output_file = f"output/{split}_extension4_evaluation.json"
