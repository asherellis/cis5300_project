...
parser = argparse.ArgumentParser(
    description="Evaluate Extension 2 (RAG with rerank) predictions."
)
parser.add_argument(
    "split",
    nargs="?",
...
extension2_output = f"output/{split}_extension2_output.json"

if not os.path.exists(extension2_output):
    print(f"Error: Output file not found: {extension2_output}", file=sys.stderr)
    sys.exit(1)

print(f"\nEvaluating Extension 2 (RAG with rerank) on {split} split...")
print(f"Questions file: {questions_file}")
print(f"Predictions file: {extension2_output}\n")

questions = load_jsonl(questions_file)
print(f"Loaded {len(questions)} questions\n")

predictions = load_predictions(extension2_output)
...
all_evaluation = {'rag_extension2': results}

output_file = f"output/{split}_extension2_evaluation.json"
