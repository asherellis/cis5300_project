...
def evaluate_llm_judge(questions, predictions, doc_dir, lambda_weight=0.5):
    ...
    if total_rubrics == 0:
        print("No questions with rubrics found. Skipping LLM-as-Judge evaluation.")
        return None
...
parser = argparse.ArgumentParser(
    description="Evaluate baseline predictions against gold standard."
)
parser.add_argument(
    "split",
    nargs="?",
...
simple_output = f"output/{split}_simple_output.json"
strong_output = f"output/{split}_strong_output.json"

simple_exists = os.path.exists(simple_output)
strong_exists = os.path.exists(strong_output)

if not simple_exists and not strong_exists:
    print(f"Error: No output files found for {split} split.", file=sys.stderr)
    print(f"  Looking for: {simple_output} or {strong_output}", file=sys.stderr)
    sys.exit(1)

print(f"\nEvaluating {split} split outputs...")
...
# Evaluate simple baseline if it exists
if simple_exists:
    print(f"{'='*60}")
    print(f"Evaluating: {simple_output}")
    print(f"{'='*60}")
    
    predictions = load_predictions(simple_output)
    print(f"Loaded predictions for {len(predictions)} questions\n")
    
    simple_results = {}
    
    retrieval_results = evaluate_retrieval(questions, predictions)
    simple_results['retrieval'] = retrieval_results
    print_results("retrieval", retrieval_results)
    
    citation_results = evaluate_citation(questions, predictions)
    simple_results['citation'] = citation_results
    print_results("citation", citation_results)
    
    all_evaluation['simple'] = simple_results
    print()

# Evaluate strong baseline if it exists
if strong_exists:
    print(f"{'='*60}")
    print(f"Evaluating: {strong_output}")
    print(f"{'='*60}")
    
    predictions = load_predictions(strong_output)
    print(f"Loaded predictions for {len(predictions)} questions\n")
    
    strong_results = {}
    
    retrieval_results = evaluate_retrieval(questions, predictions)
    strong_results['retrieval'] = retrieval_results
    print_results("retrieval", retrieval_results)
    
    citation_results = evaluate_citation(questions, predictions)
    strong_results['citation'] = citation_results
    print_results("citation", citation_results)
    
    # LLM-as-Judge evaluation (will skip if API key not set)
    # lambda_weight=0.5 means equal weight for answer quality and evidence quality
    llm_results = evaluate_llm_judge(questions, predictions, "data/corpus", lambda_weight=0.5)
    if llm_results:
        strong_results['llm_judge'] = llm_results
        print_results("llm_judge", llm_results)
    
    all_evaluation['strong'] = strong_results
    print()

output_file = f"output/{split}_evaluation.json"
