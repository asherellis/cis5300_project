...
def rag_predict(
    ...
    # these are the finalized id and cross-encoder scores
    reranked_ids, reranked_scores = rerank_encoder(question, cand_id, cand_text, k)

    retrieved_docs = [
        {
            "doc_id": doc_ids[idx],
            "score": reranked_scores[rank],  # Use cross-encoder score, not BM25
            "rank": rank + 1,
            "text": doc_texts[idx],
...
doc_dir = "data/corpus"
output_file = f"output/{split}_extension2_output.json"
