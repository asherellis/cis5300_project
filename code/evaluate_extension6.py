...
def evaluate_verification(predictions):
    """Evaluate verification-specific metrics for Extension 6."""
    results = {
    ...
    for qid, pred in predictions.items():
        status = pred.get('verification_status', 'N/A')
        attempts = pred.get('verification_attempts', 0)
    ...
    if results['total_predictions'] > 0:
        results['supported_pct'] = results['supported'] / results['total_predictions'] * 100
        results['partial_pct'] = results['partial'] / results['total_predictions'] * 100
        results['not_supported_pct'] = results['not_supported'] / results['total_predictions'] * 100
        results['avg_attempts'] = results['total_attempts'] / results['total_predictions']
    else:
        results['supported_pct'] = 0
        results['partial_pct'] = 0
        results['not_supported_pct'] = 0
        results['avg_attempts'] = 0
...
def print_results(eval_type, results):
    ...
    if eval_type == "retrieval":
        ...
        for item in results['per_question']:
            r1 = "Y" if item['recall_at_1'] else "X"
            r5 = "Y" if item['recall_at_5'] else "X"
        ...
        print(f"\n--- VERIFICATION STATUS ---")
        print(f"SUPPORTED: {results['supported']} ({results['supported_pct']:.1f}%)")
        print(f"PARTIAL: {results['partial']} ({results['partial_pct']:.1f}%)")
        print(f"NOT_SUPPORTED: {results['not_supported']} ({results['not_supported_pct']:.1f}%)")
        print(f"N/A: {results['na']}")
        print(f"\nAverage Verification Attempts: {results['avg_attempts']:.2f}")

parser = argparse.ArgumentParser(
    description="Evaluate Extension 6 (RAG with chunking + answer verification) predictions."
)
parser.add_argument(
    "split",
    nargs="?",
...
questions_file = f"data/questions/{split}_final_jsonl.txt"
extension6_output = f"output/{split}_extension6_output.json"

if not os.path.exists(extension6_output):
    print(f"Error: Output file not found: {extension6_output}", file=sys.stderr)
    sys.exit(1)

print(f"\nEvaluating Extension 6 (RAG with chunking + answer verification) on {split} split...")
print(f"Questions file: {questions_file}")
print(f"Predictions file: {extension6_output}\n")

questions = load_jsonl(questions_file)
print(f"Loaded {len(questions)} questions\n")

predictions = load_predictions(extension6_output)
...
verification_results = evaluate_verification(predictions)
results['verification'] = verification_results
print_results("verification", verification_results)
...
all_evaluation = {'rag_extension6': results}

output_file = f"output/{split}_extension6_evaluation.json"
